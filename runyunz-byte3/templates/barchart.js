...
// y labels. This runs function(name) once for each y label (name is the current label)
 ...
 d.animals = y_labels.map(function(name) {
...
// dogs with 'name' as their outcome (for this age)
  res = {name: name, y0: y0, y1: y0 + d[name], outcome: d["OutcomeType"]};
// and we also have to update y0 for the next rectangle.
 ...
 d.max = Math.max(d.animals[0].y1, d.animals[1].y1);//d.animals[d.animals.length - 1].y1;
...
var color = d3.scale.ordinal()
      .range(["#e41a1c","#377eb8"]);//(["#8dd3c7", "#ffffb3"]);//, "#bebada", "#fb8072", "#80b1d3", "#fdb462"]);
...
var outcome = svg.selectAll(".OutcomeType")
 ...
 .enter().append("g")
   .attr("class", "g")
   .attr("x_position", function (d) {return x_scale(d.OutcomeType);})
   .attr("transform", function(d, i) {return "translate(" + x_scale(d.OutcomeType)  + ",0)"; });

// this selects 'rect' which will correspond to one rect for each outcome in a bar (age)
outcome.selectAll("rect")
 ...
 .enter().append("rect")
   ...
   .on("mouseover", function(d) {
       ...
       d3.select("#tooltip")
           ...
           .select("#value")
           .text(d.y1-d.y0 + " animals were " + d.name + ", among those were " + d.outcome + ".");
...
legend.append("rect")
      ...
      .on("mouseout", function(d) {
        d3.select("#test").classed("hidden", true);
      })

// and add text 
legend.append("text")
   .attr("x", width * 0.95 - 10)
   .attr("y", height/4 + 10)
   .attr("dy", ".35em")
   .style("text-anchor", "end")
